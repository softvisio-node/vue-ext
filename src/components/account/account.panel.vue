    ...
    <CardsPanel ref="cards" @refresh="refresh">
        <template #data>
            <ext-container ref="dataCard" layout="vbox" padding="0 10 0 10" scrollable="true" viewModel="true" @ready="_ready">
                <!-- locale -->
                <ext-fieldcontainer container='{"defaults":null}' :hidden="localeHidden" :label="l10n(`Locale`, { domain: `vue-ext` })" labelAlign="left" labelWidth="200" layout='{"align":"center","type":"hbox"}'>
                    <LocaleButton/>
                </ext-fieldcontainer>

                <!-- password -->
                <ext-fieldcontainer container='{"defaults":null}' :label="l10n(`Password`, { domain: `vue-ext` })" labelAlign="left" labelWidth="200" layout='{"align":"center","type":"hbox"}'>
                    <ext-button :text="l10n(`Change password`, { domain: `vue-ext` })" @tap="_changePassword"/>
                </ext-fieldcontainer>

                <!-- email  -->
                <ext-fieldcontainer container='{"defaults":null}' :label="l10n(`Email address`, { domain: `vue-ext` })" labelAlign="left" labelWidth="200" layout='{"align":"center","type":"hbox"}'>
                    <ext-displayfield bind="{record.email}" width="200"/>

                    <!-- change email -->
                    <ext-button :text="l10n(`Change`, { domain: `vue-ext` })" @tap="_changeEmail"/>

                    <!-- confitm email -->
                    <ext-button bind='{"hidden":"{record.email_confirmed}"}' :text="l10n(`Confirm`, { domain: `vue-ext` })" @tap="_confirmEmail"/>
                </ext-fieldcontainer>
