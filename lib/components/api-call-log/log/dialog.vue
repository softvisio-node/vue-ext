<template>
    <ext-dialog :title="title" width="95%" height="95%" scrollable="true" closeAction="hide" layout="fit" viewModel="true">
        ...
        <ext-grid ref="grid" layout="fit" multicolumnSort="true" @ready="gridReady">
            <ext-column :text="i18n(`Started`)" dataIndex="started" width="150" formatter="date('Y-m-d H:i:s')"/>
            <ext-column :text="i18n(`Finished`)" dataIndex="finished" width="150" formatter="date('Y-m-d H:i:s')"/>
            <ext-column :text="i18n(`Runtime (sec.)`)" dataIndex="runtime_text" sortable="false" width="150" align="right"/>

            <ext-column :text="i18n(`Exception`)" dataIndex="is_exception_text" sorter='{"property":"is_exception"}' cell='{"encodeHtml":false}' width="100" align="center"/>
